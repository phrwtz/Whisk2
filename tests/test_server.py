...
def test_leave_message_exits_machine_learning_session_to_lobby():
    manager.reset()
    client = TestClient(app)
    ...
    with client.websocket_connect("/ws") as ws:
        ...
        _recv_type(ws, "left")
        lobby = _recv_type(ws, "lobby", max_messages=16)
        assert lobby["mode"] is None
        assert lobby["players"]["O"] is None
